(function () {
  ...
  const widgetScriptUrl = new URL(script?.src || "widget.js", window.location.href);
  ...
  function loadPaymentModule() {
    if (window.VaniPaymentRenderer?.renderPaymentForm) {
      return Promise.resolve(window.VaniPaymentRenderer);
    }
    if (paymentModulePromise) return paymentModulePromise;
    paymentModulePromise = new Promise((resolve, reject) => {
      const existing = document.querySelector("script[data-vani-payment-module]");
      const finish = () => {
        if (window.VaniPaymentRenderer?.renderPaymentForm) {
          resolve(window.VaniPaymentRenderer);
        } else {
          reject(new Error("Payment module did not initialize"));
        }
      };
      if (existing) {
        existing.addEventListener("load", finish, {once: true});
        existing.addEventListener("error", () => reject(new Error("Payment module could not be loaded")), {once: true});
        return;
      }
      const moduleUrl = new URL("widget-payment.js", widgetScriptUrl);
      moduleUrl.search = widgetScriptUrl.search;
      const scriptNode = document.createElement("script");
      scriptNode.src = moduleUrl.toString();
      scriptNode.async = true;
      scriptNode.dataset.vaniPaymentModule = "true";
      scriptNode.onload = finish;
      scriptNode.onerror = () => reject(new Error("Payment module could not be loaded"));
  ...
  function renderPaymentForm(action, context, suggestionsBox) {
    ...
    suggestionsBox.innerHTML = "";
    suggestionsBox.style.display = "grid";
    showInlineActionNotice(suggestionsBox, "Loading payment options...", "success");
    loadPaymentModule()
      .then(renderer => renderer.renderPaymentForm(action, context, suggestionsBox, {
        addMessage,
        api,
      ...
      .catch(() => {
        showInlineActionNotice(suggestionsBox, "Payment options could not be loaded. Please try again.", "error");
      });
  }
